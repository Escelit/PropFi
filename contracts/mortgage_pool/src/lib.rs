...
use propfi_types::{HealthFactor, LoanData, LoanStatus, PropertyData, PriceData};
...
#[contractimpl]
impl MortgagePool {
    pub fn initialize(env: Env, admin: Address, token: Address, property_reg: Address, oracle: Address) {
        ...
        env.storage().instance().set(&DataKey::PropertyRegistry, &property_reg);
    ...
    pub fn open_loan(env: Env, borrower: Address, prop_id: u64, amount: i128) -> u64 {
        ...
        let property_reg: Address = env.storage().instance().get(&DataKey::PropertyRegistry).unwrap();
        ...
        let total_liq: i128 = env.storage().instance().get(&DataKey::TotalLiquidity).unwrap_or(0);
        if amount > total_liq {
        ...
        env.storage().instance().set(&DataKey::TotalLiquidity, &(total_liq - amount));

        let token: Address = env.storage().instance().get(&DataKey::LiquidityToken).unwrap();
        let vault = env.current_contract_address();
        env.invoke_contract::<()>(
            &token,
            &Symbol::new(&env, "transfer"),
            Vec::from_array(&env, [vault.to_val(), borrower.to_val(), amount.into_val(&env)]),
        );
    ...
    pub fn repay(env: Env, borrower: Address, loan_id: u64, amount: i128) {
        borrower.require_auth();
        let mut loan: LoanData = env.storage().instance().get(&DataKey::Loan(loan_id)).unwrap_or_else(|| panic!("loan not found"));
        ...
        env.invoke_contract::<()>(
            &token,
            &Symbol::new(&env, "transfer"),
            Vec::from_array(&env, [borrower.to_val(), vault.to_val(), repayment.into_val(&env)]),
        );
        ...
        let total_liq: i128 = env.storage().instance().get(&DataKey::TotalLiquidity).unwrap_or(0);
        env.storage().instance().set(&DataKey::TotalLiquidity, &(total_liq + repayment));
    ...
    pub fn liquidate(env: Env, liquidator: Address, loan_id: u64) {
        ...
        env.events().publish(
            (Symbol::new(&env, "Liquidated"), loan_id),
            liquidator,
        );
    }
    ...
    pub fn deposit_liquidity(env: Env, lp: Address, amount: i128) {
        ...
        env.events().publish(
            (Symbol::new(&env, "LiquidityDeposited"),),
            (lp, amount),
        );
    }
    ...
    pub fn loan_health(env: Env, loan_id: u64) -> HealthFactor {
        let loan: LoanData = env.storage().instance().get(&DataKey::Loan(loan_id)).unwrap_or_else(|| panic!("loan not found"));
...
mod test {
    ...
    fn setup() -> (Env, Address, Address, MortgagePoolClient<'static>, Address, Address, Address) {
        let env = Env::default();
        env.mock_all_auths();
        ...
        let _prop_id = prop_reg_client.register_property(&property_owner, &100_000i128, &doc_hash, &symbol_short!("US"));
        ...
        (env, admin, property_owner, pool_client, token, prop_reg_id, oracle_id)
    }
    ...
    fn test_liquidation_health() {
        ...
        let prop_reg_client = PropertyRegistryClient::new(&env, &prop_reg_id);
        prop_reg_client.update_valuation(&1u64, &70_000i128, &oracle_id, &Symbol::new(&env, "PROP_USD"));
